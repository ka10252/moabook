...
- **스모크 QA (2026-08-17~)**: `npm run smoke` — 로그인해서 주요 화면을 돌며 JS 예외·콘솔 에러·
  Supabase 4xx·에러 바운더리를 모은다. **기능을 붙였으면 넘기기 전에 반드시 돌린다.**
  - ⚠️ **화면을 열기만 하면 부족하다.** 실제로 죽었던 것들(남의 프로필, 반납 카드 버튼)은
    전부 **한 번 더 눌러야** 드러났다. 새 진입점을 만들면 스모크에도 그 클릭을 추가한다.
  - 계정 상태에 따라 없을 수 있는 버튼은 `click(..., optional=true)` 로 둔다.
...
**사고 (2026-08-17):** 남의 프로필을 열면 "프로필을 찾을 수 없습니다"만 떴다.
원인은 `profiles_public` 뷰에서 `featured_badge`·`badges_public` 두 컬럼이 사라진 것.

무슨 일이 있었나 —
1. `20260802000008_badges.sql` 이 뷰에 배지 컬럼 두 개를 더했다.
2. `20260811000001_profiles_mrt_station.sql` 이 `mrt_station` **하나를 넣으려고**
   `DROP VIEW` → `CREATE VIEW` 하면서 **컬럼 목록을 그 이전 버전 기준으로 다시 적었다.**
3. 배지 컬럼이 조용히 빠졌다. 마이그레이션은 성공, 빌드도 통과.
   깨지는 건 그 컬럼을 select 하는 화면 하나뿐이라 아무도 몰랐다.

## 규칙
- **뷰를 고칠 때 컬럼 목록을 새로 쓰지 않는다.** 바로 위 정의를 복사해 붙이고 한 줄만 더한다.
- `DROP VIEW` → `CREATE VIEW` 를 하기 전에, **그 뷰를 만든 가장 최근 마이그레이션을 찾아 읽는다.**
  `grep -n "CREATE VIEW public.<이름>" supabase/migrations/*.sql | tail -1`
- 이 함정이 있는 공용 객체: `profiles_public`(남의 프로필을 보는 **유일한** 통로),
  `handle_new_user`(가입 트리거 — 컬럼 하나 빠지면 그 값이 영영 안 들어간다).
- 뷰는 **컬럼이 없어도 만들어진다.** 타입체크·빌드·마이그레이션 어느 것도 못 잡는다.
  잡을 수 있는 건 실제로 그 화면을 열어보는 것뿐 → `npm run smoke`.
...
# 외부 도서 API 문자열 — 반드시 엔티티를 푼다

알라딘·구글 도서는 제목/저자/소개를 **HTML에 꽂힐 것을 전제로 이스케이프해서** 준다.
그래서 『"나는 맞고 너는 틀리다"』가 `&quot;나는 맞고 너는 틀리다&quot;` 로 온다.

우리는 그 값을 React 텍스트로 그리므로(=이미 안전하게 이스케이프된다) **엔티티가 글자 그대로 보이고**,
그 상태로 등록하면 **DB에 `&quot;` 가 박혀** 검색·정렬까지 어긋난다. 화면 버그가 아니라 데이터 버그다.

- 푸는 곳은 **`src/hooks/useBookSearch.ts` 한 곳**이다. 세 소스가 전부 여기를 지난다.
  화면단에서 각자 풀면 새 화면이 생길 때마다 다시 샌다.
- `src/lib/decodeEntities.ts` 는 문자열 치환만 한다.
  ⚠️ `innerHTML` 로 푸는 흔한 요령은 쓰지 않는다 — 외부 문자열을 HTML로 해석시키는 것이라
  `<img onerror=...>` 가 그대로 실행된다.
- 회귀 검사: `npm run test:decode` (실제 소스를 트랜스파일해 검사한다)

**검색 소스를 추가하면** 매퍼에서 `decodeEntities` 를 통과시키고 `scripts/decode.test.mjs` 에
그 소스가 주는 형태를 한 줄 추가한다.

## 첫 검색만 유독 느린 이유 — Edge Function 콜드 스타트

Supabase Edge Function 은 한동안 안 부르면 잠든다. 그래서 **그 세션의 첫 검색만**
콜드 스타트를 뒤집어쓴다("갑자기 느려졌다"의 정체). 깨어 있으면 왕복 0.4~0.7초다.

검색창이 뜨는 순간 `warmBookSearch()` 로 빈 질의를 한 번 보내 깨워둔다 —
대기 시간이 사용자가 제목을 입력하는 시간과 겹쳐 사라진다.
질의가 2글자 미만이면 함수는 알라딘을 부르지 않고 즉시 빈 배열을 주므로 API 한도를 쓰지 않는다.
