...
- **사서 NPC**(커뮤니티룸=게시판 가구 있는 방에만): 아바타 시스템 기본룩, 게시판·블랙보드 사이 배치. 탭/첫 방문 자동 안내 투어(픽셀 말풍선, 탭으로 진행, 대상 가구 코랄 하이라이트). 첫 방문 1회 `moa_room_tour_seen`.
- 커뮤니티룸 우상단 '게시판' 버튼 제거(가구 클릭으로 접근), 게시판 가구에 '책장'과 동일 픽셀 라벨.
- **채팅 말풍선**: 수동 그래픽 측정 불일치로 글자 잘리던 것 → 텍스트에 자동 맞는 불투명 배경+꼬리. 캐릭터당 1개(반복 방지).
  - **불변식**: 사서/채팅 안내 UI는 캔버스 내 픽셀(별도 React 모달 아님). 사서 아바타 config는 CharacterEditor 큐레이션 옵션 내 값 사용(없는 id면 레이어 누락).
