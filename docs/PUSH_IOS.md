...
**iOS 는 앱이 떠 있을 때 온 푸시를 배너로 띄우지 않고 그냥 삼킨다.** 그냥 두면
채팅을 보는 동안 다른 대화에 온 메시지를 전혀 모른다.
→ `pushNotificationReceived` 를 받아 앱 안에서 토스트로 알린다(누르면 그 화면으로).

## 앱 아이콘의 빨간 숫자(배지)

`aps.badge` 로 **안 읽은 알림 수**를 함께 보낸다.
⚠️ iOS 는 이 숫자를 **푸시로만** 바꿀 수 있다. 앱에서 알림을 읽어도 숫자는 그대로 남고
**다음 푸시가 올 때 갱신된다.** 읽는 즉시 지우려면 배지 플러그인이 따로 필요하다 —
지금은 "대충 맞는 숫자"로 둔다.
