...
export const AuthPage = forwardRef<HTMLDivElement>((_, ref) => {
  ...
  const { user, signUp, signIn, signInWithGoogle, resendConfirmation, requestPasswordReset } = useAuth();
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      ...
      >
        ...
        <div className="bg-card rounded-3xl p-8 shadow-xl border border-border">
          ...
          {/* 구글로 시작하기 — 가입·로그인이 같은 버튼이다.
              구글이 이메일을 이미 확인해 주므로 우리 쪽 메일 인증 단계가 없다.
              비밀번호 폼보다 **위에** 둔다 — 대부분 이쪽이 빠르고, 아래 두면 안 보인다. */}
          {mode !== 'forgot' && (
            <>
              <button
                type="button"
                ...
                onClick={async () => {
                  ...
                  // 성공하면 구글로 페이지가 넘어가므로 여기 코드는 실패일 때만 이어진다
                  if (error) {
                    setGoogleLoading(false);
                    toast.error('구글 로그인을 시작하지 못했어요. 잠시 후 다시 시도해주세요.');
                  }
                }}
                className="w-full h-12 flex items-center justify-center gap-2.5 rounded-xl border border-border bg-card text-[15px] font-semibold text-foreground hover:bg-muted/50 transition-colors disabled:opacity-60"
              >
                {googleLoading ? (
                ...
                구글로 {mode === 'signup' ? '가입하기' : '로그인'}
              </button>

              <div className="flex items-center gap-3 my-5">
                <span className="flex-1 h-px bg-border" />
                <span className="text-xs text-faint">또는 이메일로</span>
                <span className="flex-1 h-px bg-border" />
              </div>
            </>
          )}
